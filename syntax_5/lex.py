...
# For bad characters, we just skip over it
        ...
        with open(filename,'r',encoding='utf-8') as file: # input
            ...
            for i in range(0,len(dataline)):
                  if len(dataline[i])>10000:
                        print(f"{'ERROR':^20}")
                        print(f"{'code:A-01'!s:<10}  {f'line:{str(i+1)}'!s:<3}   {'value:代码行长度过长'!s:<10}")
                        raise TypeError("code:A-01 value:代码行长度过长 %d" % (i+1))
...
if __name__ == '__main__':
     lexi=Lexer()
     lexi.build('comment.pas')
     #lexi.debug('lex_test\id.pas',1)D:\onedrive\OneDrive\桌面\学习\orders\pythonyacclex\syntax_3\lex.py
