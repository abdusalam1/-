    ...
    def child(self, name):
        ...
        def p_const_declerations(p):
            ...
            for i in p[2]['constant']:
                # 声明的const量名与形参名相同
                find = 0
                for key, value in SymbolTable[-1]["para"].items():
                    if key.count(i.constId) != 0:
                        find = 1
                if find == 1:
                    self.error.append({
                        'type': '常量名与形参名重复',
        ...
        def p_statement_variable_assign(p):
            ...
            for i in range(len(SymbolTable) - 1, -1, -1):
                vp = SymbolTable[i]['const'].copy()
                if vp.get(p[1].varId)!=None:
                    self.error.append({
                        'type': '修改常量错误',
                        'line': p.lineno(2),
                        'value': p[1].varId+'是常量'
        ...
        # 用来判断p节点的实参和符号表中函数形参是否一致
        def compare(p,id,lineno):
            num = 0     # 形参数目
            # 处理递归调用
            if SymbolTable[-1]["id"] == id:
                offset = 0
                for i, j in SymbolTable[-1]["para"].items():
                    count = len(i)
                    while count > 0:
                        if Exp_type(p[offset - count + len(i)]) != j:
                            self.error.append({
                                'type': '形参与实参类型不同',
                                'line': lineno,
                                'value': id
                ...
                return 1
            #   处理非递归调用
            for i in SymbolTable[0]["subF"].get(id)[0].keys():
                num += len(i)
            if len(p) != num:
                self.error.append({
                    'type': '形参与实参数量不同',
                ...
                return 0
            offset = 0
            for i, j in SymbolTable[0]["subF"].get(id)[0].items():
                count = len(i)
                while count > 0:
                    if Exp_type(p[offset - count + len(i)]) != j:
                        self.error.append({
        ...
        def p_procedure_call_no_arg(p):
            ...
            # 排错
            if p[1] != SymbolTable[-1]['id'] and  SymbolTable[0]['subF'].get(p[1])==None:
                self.error.append({
                    'type': '过程不存在',
                    'line': p.lineno(1),
                    'value': p[1]
                })
            elif SymbolTable[-1]['subF']!={} and SymbolTable[0]['subF'].get(p[1])[1] != 'void':
                self.error.append({
                    'type': '这是函数而非过程',
                    'line': p.lineno(1),
                    'value': p[1]
        ...
        def p_expression_with_relational_op(p):
            ...
            if (fac.value == 'error'):
                self.error.append({
                    'type': '表达式错误',
                    'line': p[1].line,
                    'value': str(p[1].value) + " != " + str(p[3].type)
                })
            p[0] = fac
