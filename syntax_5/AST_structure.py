...
class SubProgram:
    ...
    """
    def __init__(self,a,b,c,d):
        self.constList: dict = a if a!=None else {}  # 常量定义列表
...
class Type:
    ...
    def __init__(self,a='',b=0,c=0,d=0,e=0):
        self.type:str = a       #基本类型，int、char、float、boolean
        self.line:int = b       #行号
        self.arrFlag:bool= c    #0不是数组，1是数组
        self.lB:list[int] = d  #数组下界（n维）
        self.uB:list[int] = e  #数组上界
...
class Expression:
    ...
    def __init__(self,ty,line,value=''):
        ...
        self.value=value #compound 则为表达式数值类型 ；其余为其值
...
def Exp_type(exp:Expression):
    if exp==None: return 'none'
    if exp.type == 'var' :
        return exp.varRef.type
    elif exp.type == 'function':
        if SymbolTable[-1]['subF'].get(exp.value) :
            return SymbolTable[-1]['subF'].get(exp.value)[1]
        else: #本函数名
             return SymbolTable[-1]['type']   
    elif exp.type=='compound':
         if exp.opType=='damn':
            return Exp_type(exp.subE1)
         else:
            type1=Exp_type(exp.subE1)
            type2=Exp_type(exp.subE2)
            if type1==type2:
                return type1
            else:
                return 'error'
