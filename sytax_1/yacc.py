...
class parser:
    ...
    def p_program_body(p):
        ...
        p[0]["symbolTable"] = SymbolTable(
            "program_body", p[1]["constant"], p[2]["Variant"], p[3]["symbolTable"]
        )
    ...
    def p_idlist(self, p):
        ...
        if len(p) == 2:
            ...
            if p[1].get(p[3]) == None:  # 判断重复
                ...
                self.error.append({"type": "变量重复", "line": p.lineno(3)})
    ...
    def p_const_decleration(p):
        ...
        # 符号表
        p[0]["symbolTable"] = {
            p[1]: Constant(p[1], p.lineno(1), p[3]["type"], p[3]["value"])
        }

    def p_const_decleration_2(self, p):
    ...
    def p_const_value_addop(p):
        ...
        |    ADDOP DIGITS"""
        p[0] = {"type": str(type(p[2]))[8:-2], "value": p[2] if p[1] == "+" else -p[2]}

    def p_const_value_num(p):
        """const_value :  NUM
        |     DIGITS"""
        p[0] = {"type": str(type(p[2]))[8:-2], "value": p[1]}
